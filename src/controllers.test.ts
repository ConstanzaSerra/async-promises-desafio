...
test("Testeo la búsqueda de un contacto", (t) => {
  ...
  return controlador2.promesa.then(() => {
    const contacto = controlador2.processOptions({
      action: "get",
      params: { id: 1 },
    });
...
test.serial("Testeo la búsqueda de todos los contactos", (t) => {
  ...
  return controlador3.promesa.then((contactosEsperados) => {
    const contactos = controlador3.processOptions({
      action: "get",
      params: "",
    });

    return jsonfile.readFile(__dirname + "/contacts.json").then(() => {
...
test.serial("Testeo la grabación de un nuevo contacto", (t) => {
  ...
  return controlador4.promesa.then(() => {
    ...
    return controlador4
      ...
      .then(() => {
        const contactosActualizados = controlador4.processOptions({
          action: "get",
          params: "",
        });
